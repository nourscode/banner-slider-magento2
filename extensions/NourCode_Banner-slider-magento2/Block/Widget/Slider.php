 ...
 * NourCode
 * @category    NourCode
 * @copyright   Copyright (c) 2014 NourCode (http://www.magepow.com/)
...
namespace NourCode\Bannerslider\Block\Widget;
...
{
    const DEFAULT_CACHE_TAG = 'NOURCODE_BANNERSLIDER';
    const MEDIA_PATH = 'nourcode/bannerslider';
    ...
    protected function _construct()
    {
        ...
        $this->_bannerslider = $this->_bannersliderFactory->create()->getCollection()->addFieldToSelect('*')
                        ->addFieldToFilter('stores',array( array('finset' => 0), array('finset' => $store)))
                        ->addFieldToFilter('status', 1)
                        ->addFieldToFilter('identifier', $identifier)
                        ->setOrder('stores', 'desc')
                        ->setOrder('bannerslider_id', 'desc')
                        ->setPageSize(1)
                        ->getFirstItem();
        if (!$this->_bannerslider){
            echo '<div class="message-error error message">Identifier "'. $identifier . '" not exist.</div> ';
            return;
        }
        if (!$this->_bannerslider->getStatus()){
            return;
        }
        $data = json_decode($this->_bannerslider->getConfig(), true);
    ...
    public function getCacheKeyInfo()
    {
        ...
        $keyInfo[]   =  $this->getBannerslider()->getId();
        return $keyInfo;
    }
    ...
    {
        return [self::DEFAULT_CACHE_TAG, self::DEFAULT_CACHE_TAG . '_' . $this->getBannerslider()->getId()];
    }
    ...
    public function getMediaUrl($file)
    {
        ...
        return $this->_urlMedia .'nourcode/bannerslider'. $file;
    }
